...
/**
 * Wires vanilla player-lifecycle + chat events to {@link DiscordService}. Logic-free
 * delegation. Registered on the game event bus for BOTH dists (no Dist filter) so it
 * runs on a dedicated server and the singleplayer integrated server alike — these are
 * all server-side events. The client-only network-consent prompt lives separately in
 * {@code ClientPresenceEvents}.
 */
